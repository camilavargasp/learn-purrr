...
*type --> defines what kind of outout I will get
...
map2 = allowa you to build a list of thing with different objects.
Outcome: 
...
You can also do this with three elements!!

How does this is different thanmutate?


#### Programming with Functions
*having functions as the core of your programming
...
```{r}
#waht happens inside a function stays inside the function (eg:variables created inside the fucntion stay inside the function adn are not part of the environment). Be careful with ehat you put inthe function!! ONLY THINGS YOU ARE GOING TO EXTRACT/PRINT
z <- 10
...
```{r}
got_chars[1:5] %>%
  map(`[`, c('name', 'allegiances')) #`[` is using brakets as a function. This is saying print names and allegiances
...
*Give me the Lannister names
```{r}
got_chars %>%
  set_names(map_chr(.,'name')) %>%
  map(`[`,c('name','allegiances')) %>%
  purrr::keep(~stringr::str_detect(.$name, 'Lannister')) %>% #keep is the purrr version of filter
  listviewer::jsonedit()
```

*inside map or map2 you can use .x indicate what goes first.
.y indicates what goes second

*When you have multiple list to commine you use pmap and to give an order you have to right out the elements.
...
. when using the pipe, mena the thing that was pased though the pipe.
...
got_chars %>%
  set_names(map_chr(.,'name'))  %>%
  map(`[`,c('name','allegiances')) %>%
  listviewer::jsonedit() #listviewer is a way to simple sii your list!

```

##D

`safely()` purr function = for every element the map is retuning it gives me where is the error. You can extract information on why several numbers are not working.
See where the problem is

browser() to check inside the function what is going wrong
...
*four different models I want to try
```{r}
...
Now, let's use purrr to convert each of these character strings into a model (formula)


```{r}

model_frame <- model_frame %>% 
    mutate(model = map(model, as.formula)) #maping over model and maping for each formula
...
model_frame <- model_frame %>%
  mutate(fit = lm(model, data = gapminder)) #it does not woek because lm does not no how to deal with multiple models
...
#you can directly plot form this outome!
...
```{r}
#testing the predicting power of each of the model without relying in the AIC

validate <- gapminder %>%
 rsample::vfold_cv(10) #split data to 10 training set data. 

test_data <- list(test_training = list(validate), model_name = model_frame$model_name)  
  
test_data <- cross_df(test_data) %>% #gives you the factoria combination of your variables
